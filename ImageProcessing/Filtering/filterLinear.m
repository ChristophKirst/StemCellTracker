function imgf = filterLinear(img, ker, varargin)
%
% imgf = filterLinear(img, ker, varargin)
% imgf = filterLinear(img, type, ksize, varargin)
% imgf = filterLinear(img, ..., padding)
%
% description:
...
if dim < 2 || dim > 4
   error('filterLinear: img must be a 2d or 3d gray scale img')
end
...
if ndims(ker) ~= dim
   error('filterLinear: img and kernel dimensions do not agree')
end
if dim == 3 && size(img,3) == 3
   error('filterLinear: img must be gray scale img')
end
