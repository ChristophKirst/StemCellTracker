function [V,S] = detectAlphaVolume(X,R,plt)
%
...
%   R    (optional) probe radius, R = Inf returns convex hull (R = Inf)
%   plt  (optional) plot the result
%
% output:
%    V   volume
%    S   struct with entries
%        .tri - Triangulation of the alpha shape (Mx3 or Mx4)
%        .vol - Area or volume of simplices in triangulation (Mx1)
%        .rcc - Circumradius of simplices in triangulation (Mx1)
%        .bnd - Boundary facets (Px2 or Px3)
%

%   % 2D Example - C shape
%   t = linspace(0.6,5.7,500)';
%   X = 2*[cos(t),sin(t)] + rand(500,2);
%   subplot(221), detectAlphaVolume(X,inf,1);
%   subplot(222), detectAlphaVolume(X,1,1);
%   subplot(223), detectAlphaVolume(X,0.5,1);
%   subplot(224), detectAlphaVolume(X,0.2,1);
%
%   % 3D Example - Sphere
%   [x,y,z] = sphere;
%   [V,S] = detectAlphaVolume([x(:),y(:),z(:)]);
...
%   X = [x(ii),y(ii),z(ii)];
%   X = [X; 0.8*X];
%   subplot(211), detectAlphaVolume(X,inf,1);
%   subplot(212), detectAlphaVolume(X,0.5,1);
%
...
if nargin < 3, plt = 0; end

% Check coordinates
dim = size(X,2);
if dim < 2 || dim > 3
    error('detectAlphaVolume:dimension','X must have 2 or 3 columns.')
end

% Check probe radius
if ~isscalar(R) || ~isreal(R) || isnan(R)
    error('detectAlphaVolume:radius','R must be a real number.')
end
