function [shift, quality] = align2ImagesByCorrelation(img1, img2, varargin)
%
% [shift, quality] = align2ImagesByCorrelation(img1, img2)
%
% description: 
%     globally align two images using overlap weighted cross correlation calculated via fft
%
% input:
%     img1,img2    images
...
dim = length(s1);
if dim ~= length(s2)
   warning('alignGlobally2ImagesByCorrelation: image dimension mismatch: %g ~= %g, padding with ones!', length(s1), length(s2));
...
if minovl <= 0
   warning('alignGlobally2ImagesByCorrelation: overlap.min has to be at least 1!');
   minovl = 1;
end
