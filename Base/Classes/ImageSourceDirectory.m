 classdef ImageSourceTagged < ImageSource
   ...
   properties 
      ...
      iinfocommand   = '';  % tagged command to get ImageInfo of each tagged individual data / image (e.g. 'imreadBFInfo(<file>)') should always retunr consistent image format keeping singelton dimensions !
...
%             end
      ...
      function obj = fromFileList(obj, fname, varargin)
         ...
         obj.ireadcommand   = getParameter(param, 'command', 'imreadBF(''<file>'')');
         obj.iinfocommand   = getParameter(param, 'infocommand', 'imreadBFInfo(''<file>'')');
      ...
      function obj = fromReadCommandAndFileExpression(obj, varargin)
         ...
         if isempty(infocommand)
            if ~isempty(strfind(readcmd, 'imreadBF'))
               infocommand = strrep(readcmd, 'imreadBF', 'imreadBFInfo');
            else
               infocommand = 'imreadBFInfo(''<file>'')'; % best guess
            end
         end
