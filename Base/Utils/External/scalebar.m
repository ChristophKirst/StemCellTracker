...
%  - added unit string
    ...
    if scalelength==0
        sl = range2(axeslims(:,1))*scalelengthratio;
    ...
    if ischar(location)
        switch location
            case 'northeast'
                anchor = [axeslims(2,1) - axesdir(1)*range2(axeslims(:,1))*0.05, ...
                          axeslims(2,2) - axesdir(2)*range2(axeslims(:,2))*0.05];
                direction = 'southwest';
            case 'northwest'
                anchor = [axeslims(1,1) + axesdir(1)*range2(axeslims(:,1))*0.05, ...
                          axeslims(2,2) - axesdir(2)*range2(axeslims(:,2))*0.05];
                direction = 'southeast';
            case 'southwest'
                anchor = [axeslims(1,1) + axesdir(1)*range2(axeslims(:,1))*0.05, ...
                          axeslims(1,2) + axesdir(2)*range2(axeslims(:,2))*0.05];
                direction = 'northeast';
            case 'southeast'
                anchor = [axeslims(2,1) - axesdir(1)*range2(axeslims(:,1))*0.05, ...
                          axeslims(1,2) + axesdir(2)*range2(axeslims(:,2))*0.05];
    ...
    line(ends(:,1,2), ends(:,2,2), 'colormchannellabel(', colour, 'linewidth', linewidth, 'parent', hg);
    ...
    udata = {'ScaleLengthRatio',scalelengthratio;...
             'AnchorRatio',[(anchor(1)-min(axeslims(:,1)))/range2(axeslims(:,1)) (anchor(2)-min(axeslims(:,2)))/range2(axeslims(:,2))];...
             'Colour',colour;...
             'Listeners',hL;...
             'Bold',boldflag};
