function [result] = bfopenEDS(id, varargin)
% Open microscopy images using Bio-Formats. 
% Temporary hack by EDS to specific which colony to open
%
% SYNOPSIS r = bfopen(id)
%          r = bfopenEDS(id, colony)
%          r = bfopen(id, x, y, w, h)
%
% Input
%    r - the reader object (e.g. the output bfGetReader)
%
%    colony - colony number or list of colonies. If nargin==2 then assume
%    second argument is colony. Nargin==2 precludes inputting one of
%    x,y,w,h.  Output is still result{1:total_colonies, 4} with
%    result{~=colony, :} = []; Colonies are 1,2,..numSeries
%
...
if planeSize/(1024)^3 >= 2,
    error(['Image plane too large. Only 2GB of data can be extracted '...
        'at one time. You can workaround the problem by opening the plane in tiles.']);
end

numSeries = r.getSeriesCount();
% mods of EDS are below.
ncolony = 1:numSeries;
if nargin == 2
    if 1 <= min(varargin{1}) && max(varargin{1}) <= numSeries
        ncolony = varargin{1};
    else
        disp(varargin{1});
        error('max colonies= %d. Input colony list with out of bounds entries--bye\n', numSeries);
    end
    varargin = {};  % kill varargin since not inputting x,y,w,h
end
result = cell(numSeries, 2);

for s = ncolony    %%% 1:numSeries
