...
% Yuanjie Zheng modified it to display titles of subfigures on April 15, 2010 
% @PICSL@UPenn
%
% Examples:
%   imdisp
...
% Parse inputs
% changed by Zheng to incoude titles
[map layout gap indices lims titles] = parse_inputs(varargin);
...
elseif n == 1
    ...
    for a = 1:layout(1)
        for b = 1:layout(2)
            ...
            if ~isempty(titles{c})% changed here by Zheng
                title(titles{c});
            end
        end
    end
...
%% Parse inputs
% this function was changed by Zheng for parsing input titles
function [map layout gap indices lims titles] = parse_inputs(inputs)
