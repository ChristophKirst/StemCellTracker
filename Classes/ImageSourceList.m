classdef ImageSourceList < ImageSource
   %
   % ImageSourceList class represents an image composed of a list of images such as a Stack or a TimeSeries
   % 
   ...
   methods
      function obj = ImageSourceList(varargin) % constructor
         %
         % ImageSourceList()
         % ImageSourceList(list)
         % ImageSourceList(...,fieldname, fieldvalue,...)
         %

         if nargin == 1
            if isa(varargin{1}, 'ImageSourceList') %% copy constructor
               obj = copy(varargin{1});
