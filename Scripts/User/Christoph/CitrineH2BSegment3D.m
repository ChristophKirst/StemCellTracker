function imgseg = CitrineH2BSegment3D(img, imgmask, verbose, time)


%% Filter

imgmed = filterMedian(img, [3, 3, 3]);
imgmed = immask(imgmed, imgmask);

if verbose   
   figure(4)
   set(gcf, 'Name', ['Time: ', num2str(time), ' Masked Median']);
   implottiling(imgmed, 'tiling', [4,3]);
end
...
imgmax = imextendedmax(mat2gray(imgf), 0.01);
...
%    implot3d(mat2gray(imgmax));
   ...
   %size(stackc);
   stackc(:,:,:,2:3) = 0;
   stackc = stackc + gray2rgb(mat2gray(img));
   
   figure(15); clf
   set(gcf, 'Name', ['Time: ', num2str(time), ' Seeding']);
   implottiling(stackc, 'tiling', [4,3]);
