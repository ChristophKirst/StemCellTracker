%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Citrine Movie - Freds data for Eric / Colony 2
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
texpmip = '/home/siggia/TGMM/Smad4-GFP_trial_quantification/Colony2/W1F<F,3>T<T,4>Z<Z,2>C<C,1>.tif';
fns = tagExpressionToFiles(texpmip)

tagExpression(fns, 'tagnames', num2cell('FTC'))

%%
clc
ismip = ImageSourceFiles(fns, 'tagnames', num2cell('FTC'));
ismip.printInfo

%%
clc
ismip.resetRange;
ismip.setReshape('F', 'UV', [2,2]);
ismip.setCellFormat('UvTC');
...
% c = 1;
% 
% imgf = zeros(is.dataSize);
% imgb = inf * ones(is.dataSize);
% 
% k = 1;
% 
% times = is.fileTagRange.T
% fields = is.fileTagRange.F
% 
% slices = [1];
...
%          fn = is.fileExpression;
%          fn = tagExpressionToFileExpression(fn, 'T', times(t), 'C', c, 'Z', 0, 'F', fields(f));
%          imgs{f} = imreadBF(fn);
%       end
...
load '~/Desktop/Smad4Colonies/correction.mat'

texpout = '~/Desktop/Smad4Colonies/Colony2/T<T,4>C<C,1>.tif'

times = [12, 18, 20, 25, 30, 35, 40, 60, 90, 120, 150];
%times = 1;
slices = [1];
...
for t = 1:length(times)
   ...
   fnout = tagExpressionToString(texpout, 'C', cc, 'T', tt, 'Z', zz-1);
