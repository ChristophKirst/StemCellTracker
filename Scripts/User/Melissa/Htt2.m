...
exp = Experiment('name', 'Melissa', 'description', 'some random segmentation for Melissa',...
                 ...
                 'ImageDirectoryName', '140513_RUES2Q150_500um_<sample>.tif_Files/',...
                 'ResultDirectoryName', 'Results/', ...
                 'ReadImageCommandFormat', 'imread(''<file>'')',...
                 'ReadImageFileFormat', '140513_RUES2Q150_500um_<sample>_p<tile,1>c<channel,1>.tif');
...
imgs = medianFilter(img, 4);
%%
imgs = mat2gray(imgs);

imgmask = imgs > 0.1;
imgmask = imopen(imgmask, strel('disk', 20));
imgmask(:, 2000:end) = 0;
imgmask = and((imgseg ~= 1), imgmask);
...
%%
imgss = imopen(double(imgseg==2), strel('disk', 2));
imgss = gaussianFilter(imgss, 15);
imgss = mat2gray(imgss);
imgss = immask(imgss, imgmask);

figure(70); clf;
implot(imgss)

%%
%imgss = diskFilter(imgss, 15, 0, 1, -1);
%imgss = sphereFilter(imgs, 15);

imgcl = imextendedmax(imgss, 0.1);
%imgcl = imregionalmax(imgss);
imgcl = immask(imgcl, imgmask);
imgcl = imdilate(imgcl, strel('disk', 2));
%imgcl = imopen(imgcl, strel('disk', 2));

%imgcl = imdilate(imgcl, strel('disk', 2));
imgcl = bwlabeln(imgcl);
%imgcl = postProcessSegments(imgcl, 'volume.min', 100);
