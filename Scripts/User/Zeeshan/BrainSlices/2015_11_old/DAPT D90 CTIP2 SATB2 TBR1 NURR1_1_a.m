...
imgmaskHi1 = imgsSt{5} > 1.9;
%imgmaskHi1 = imdilate(imgmaskHi1, strel('disk', 3));
%imgmaskHi1 = postProcessSegments(bwlabeln(imgmaskHi1), 'volume.max', 200, 'fillholes', false) > 0;
imgmaskHi1 = not(imgmaskHi1);

imgmaskHi2 = imgsSt{5} > 3; %0.7;
imgmaskHi2 = imclose(imgmaskHi2, strel('disk', 2));
%imgmaskHi2 = postProcessSegments(bwlabeln(imgmaskHi2), 'volume.min', 5, 'fillholes', false) > 0;
...
if verbose
   figure(3); clf;
   set(gcf, 'Name', 'BM')
   implottiling({imgC; imgsSt{5}; mat2gray(sum(imgC,3)); mat2gray(imgI)}, 'titles', {'imgC', 'DAPI', 'sum', 'imgI'} )
end
...
imgWs = imimposemin(iminvert(imgI), imgmax);
...
imgSP = imgS;
imgSP = postProcessSegments(imgS, imgI, 'intensity.median.min', 0.1, 'volume.min', 50, 'volume.max', 150, 'fillholes', false);
...
if verbose 
   statsSize = imstatistics(imgSP, 'Volume');
   figure(7); clf;
   hist([statsSize.('Volume')], 128)
   title('Cell Size')
end


%%
imgSP2 = postProcessSegments(imgSP, imgsSt{5}, 'intensity.median.min', 0.05, 'intensity.median.max', 0.6);

if verbose %> 1
   imgsurf = impixelsurface(imgSP2);
   figure(50); clf;
   set(gcf, 'Name', 'Cells Postprocessed on DAPI')
   implottiling({imoverlaylabel(imgsSt{5}, imgsurf, false);imoverlaylabel(imgC, imgsurf, false)})
...
if verbose
  figure(80); clf;
  set(gcf, 'Name', 'Cells after Selection')
 
  imgsurf = zeros(size(imglab));
  for i = 1:length(stats)
     imgsurf(stats(i).PixelIdxList) = i;
  end
  
  imgsurf = impixelsurface(imgsurf);
  implottiling({imoverlaylabel(imgsSt{5}, imgsurf, false);imoverlaylabel(imgC, imgsurf, false)})
...
save(statsfile, 'chlabel', 'stats', 'statsCh', 'statsChN',  'neuroClass');
%load(statsfile)
